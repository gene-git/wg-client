...
def get_ssh_port_prefix(pfx_range: list[str]) -> int:
    ...
    pfx_ints = [int(num) for num in pfx_range]
    if len(pfx_ints) == 1:
        return pfx_ints[0]

    num = random.randint(pfx_ints[0], pfx_ints[1])
    return num


def ssh_args(wg_ip: str, server: str, prefix: int) -> tuple[str, str, str, str]:
    '''
    returns:
...
def ssh_listener_args(test: bool, wg_ip: str, host: str, prefix: int) -> list[str]:
    """
